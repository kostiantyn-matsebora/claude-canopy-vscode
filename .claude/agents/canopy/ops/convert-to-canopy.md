...
4. Read `shared/project/ops.md`, `shared/framework/ops.md`, and all `shared/project/<category>/` files.
   - For each candidate op identified in step 2: if an equivalent already exists in shared, mark it as a shared reference — do not add it to the skill-local `ops.md`
   - For each candidate resource file: if equivalent content already exists in a shared category file, mark it as a shared reference — do not create a duplicate
