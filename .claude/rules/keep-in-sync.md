...
|---|---|
| New framework primitive added in canopy's [`skills/canopy-runtime/references/ops/<slice>.md`](https://github.com/kostiantyn-matsebora/claude-canopy/blob/master/skills/canopy-runtime/references/ops/) (sliced in v0.21.0; the index lives at `references/ops.md`) | Add to `RESERVED_PRIMITIVES` in `diagnosticsProvider.ts`; add `PRIMITIVE_DOCS` entry (with `slice:` field) in `opRegistry.ts`; add `case` in `checkPrimitiveSignatures()`; add to `primitive-control` or `primitive-action` regex AND op-call exclusion in `syntaxes/canopy.tmLanguage.json`; consider a `snippets/canopy.json` snippet; if the primitive belongs to a new slice, extend the `PrimitiveSlice` union and `CANOPY_FEATURE_VALUES` set in `canopyDocument.ts` |
