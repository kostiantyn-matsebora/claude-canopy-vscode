...
If the `## Agent` body uses shape (C) — `**explore** — execute NAMED_OP` — resolve `NAMED_OP` via the standard op lookup chain (skill-local `<skill>/ops.md` → consumer-defined cross-skill ops if any → `.github/skills/canopy/references/framework-ops.md` for primitives), read the op body, and execute it inline as the fallback procedure.
...
3. `.github/skills/canopy/references/framework-ops.md` — framework primitives (always available when `canopy` is installed)
