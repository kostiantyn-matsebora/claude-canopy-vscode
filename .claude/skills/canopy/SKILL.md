...
* canopy
  ...
  * IF << context.platform == "claude"
    * Read `references/runtime-claude.md` for platform execution rules
  * ELSE
    * Read `references/runtime-copilot.md` for platform execution rules
...
- Framework primitives (IF, ELSE_IF, ELSE, SWITCH, CASE, DEFAULT, FOR_EACH, BREAK, END, ASK, SHOW_PLAN, VERIFY_EXPECTED) are never defined in skill or project ops — they live in `references/framework-ops.md`
- Before creating any op or resource file, always read `references/framework-ops.md` and any existing project-wide ops the consumer has defined — reference shared content, never duplicate it
...
- Always load the platform runtime spec (`references/runtime-claude.md` or `references/runtime-copilot.md`) before executing any op procedure
