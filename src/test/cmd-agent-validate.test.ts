...
import { agentValidate } from '../commands/canopyAgent';
...
describe('canopy.agentValidate', () => {
  it('happy path: picks skill and dispatches /canopy validate the <skill> skill', async () => {
    vi.spyOn(vscode.window, 'showQuickPick').mockResolvedValue(
      { label: 'probe' } as unknown as vscode.QuickPickItem,
    );
    await agentValidate();
    expect(sendText).toHaveBeenCalledTimes(1);
    expect(sendText.mock.calls[0][0]).toBe('claude "/canopy validate the probe skill"');
  });
