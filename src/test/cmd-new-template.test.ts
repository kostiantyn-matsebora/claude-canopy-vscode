...
import { newTemplate } from '../commands/newResource';
...
beforeEach(() => {
  ...
  // existsSync: SKILL.md exists, target template file doesn't (so createAndOpen writes).
...
describe('canopy.newTemplate', () => {
  it('happy path: prompts for name and ext, writes assets/templates/<name>.md', async () => {
    vi.spyOn(vscode.window, 'showInputBox').mockResolvedValue('payload');
    ...
    expect(writtenPath).toMatch(/assets\/templates\/payload\.md$/);
