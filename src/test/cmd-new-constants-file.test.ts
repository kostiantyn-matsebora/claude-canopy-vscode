...
import { newConstantsFile } from '../commands/newResource';
...
beforeEach(() => {
  ...
  // Single workspace folder (not strictly needed when active editor resolves dir).
  ...
  // Default existsSync: SKILL.md exists, target constants file does not.
...
describe('canopy.newConstantsFile', () => {
  it('happy path: writes assets/constants/<name>.md with constants table content', async () => {
    vi.spyOn(vscode.window, 'showInputBox').mockResolvedValue('thresholds');

    await newConstantsFile();
    ...
    expect(writtenPath.endsWith('assets/constants/thresholds.md')).toBe(true);
    // Content carries the constants template heading (interpolated name) and table header.
    expect(writtenContent).toContain('# thresholds');
    expect(writtenContent).toContain('| Name | Value | Description |');
