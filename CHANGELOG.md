...
- **Static type-flow diagnostics** (`diagnosticsProvider.ts`):
  - **`checkUniversalContractMarkers`** — for every op in `ops.md` carrying contract markers without the `> **Subagent.**` lead, verify the schema files exist and that the schemas' top-level `properties` cover the op's `<<` / `>>` named-field signature. Subagent ops continue to be covered by `checkSubagentMarkerDefs` to avoid double-flagging.
